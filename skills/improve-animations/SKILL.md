...
description: Survey a codebase's animation and motion code as a senior motion advisor, then produce a prioritized audit and self-contained implementation plans for other agents (or cheaper models) to execute. Read-only on source code — it plans improvements, it does not apply them. Use when the user asks to "improve the animations", "audit the motion", "make this app feel better", or wants a roadmap of animation fixes rather than a review of a single diff. For a lighter one-shot sweep for missing animation (nothing existing to fix, just "what could be animated here?"), use find-animation-opportunities instead — this skill's own audit surfaces missed opportunities too, but only as a byproduct of judging existing motion, not a dedicated sweep.
---
...
It does ONE thing: survey *existing* animation and motion code for quality problems, then produce prioritized findings and implementation plans. It does not review a single diff (that's `review-animations`), it does not implement fixes itself, and it is not the tool for finding places that don't animate yet but should — that dedicated, gate-driven sweep is `find-animation-opportunities`. Route by what's being judged: code that already animates and might be doing it wrong → this skill; a surface with no motion at all being scanned for candidates → `find-animation-opportunities`.
...
After the table, list 2–4 **missed opportunities** — places that don't animate but should (a jarring state change, a rare delight moment) — separately, since they're additive rather than corrective. This is a byproduct of the audit, capped and lightweight; if the user actually wants a dedicated, gate-driven sweep for missing motion, hand off to `find-animation-opportunities` instead of expanding this list.
