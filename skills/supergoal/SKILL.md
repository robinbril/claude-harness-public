...
description: >-
  ...
  het token-zuinige alternatief voor hetzelfde doel is keten-loop. Plans via grill-intake and recon, gets one confirmation,
  then hands off a single ready-to-paste `/goal` that runs the whole chain. Works on Claude Code
  and Codex.
...
1. **Intake**: de beslisboom afgrillen tot gedeeld begrip, één scherpe vraag per keer.
...
6. **Plan review**: menselijke gate, met self-critique en HTML-overzicht.
7. **Dispatch**: één kant-en-klare `/goal`; daarna draait de generator/evaluator/council-loop autonoom tot de final audit.

Twee vaste menselijke gates (Stage 1 en Stage 6), plus de dynamische council-gate die de run alleen pauzeert wanneer alleen de gebruiker de richting kan kiezen.
...
Werk de beslisboom af met **één scherpe vraag per keer**, wacht op het antwoord (elk antwoord bepaalt de volgende vraag; niet batchen). Regels:
...
Roep dan `AskUserQuestion` aan, header "Start chain?": **Start now** (pre-flight, dan de `/goal` regel) / **Adjust an assumption** / **Tweak a phase** / **Restructure phases**. Bij een revisie-optie: pin met een vervolg-vraag precies vast wat, pas toe, herdraai `validate-phase.sh`, toon de samenvatting opnieuw. Loop tot "Start now" of afbreken. Dispatch nooit op stilte.

---

## Stage 6.5 - Pre-flight smoke check

Na "Start now", vóór het `/goal` blok: draai de mandatory commands (gededupliceerd) één keer tegen de huidige staat. Dit vangt een startpunt dat al stuk is.
...
Kies langs de Dispatch-tiers, op twee assen: complexiteit van de vraag en budget. Budget: geen Workflow-tool = zuinig; anders vraag het één keer (zuinig / standaard / max). Triviaal (< 1 uur, één bestand): zeg dat dit geen Supergoal nodig heeft. Terugkerend: `/loop` of scheduled task (tier 4). Al het andere: één `/goal` met subagents per scheidbaar spoor (tier 1), plus Workflow-swarms per fase alleen bij max budget (tier 3).

Leg vast in `STATE.md` als `Dispatch: <goal | loop> + <subagents | workflow>` (dit veld begrenst alleen de Workflow-tier; tier-1 subagents draaien altijd bij scheidbaar werk). Stage 7 print de bijbehorende plak.

---

## Stage 7 - Dispatch (één plak)

Slash commands vuren alleen vanuit gebruikersinput, dus dit is een eerlijke één-plak overdracht. Na "Start now":

1. Update `STATE.md`: `Status: READY_TO_DISPATCH`, `Current phase: 1`, `Baseline ref:` op `git rev-parse HEAD 2>/dev/null || echo "no-git"`. Initialiseer `Phase baselines:` (leeg); de generator voegt per fasegrens `phase <N> pre: <ref>` toe, de rollback-ankers uit `phase-N-rationale.md`.
2. Kopieer naar `.supergoal/`: `templates/PROTOCOL.md` (uitvoeringsloop), `prompts/phase-judge.md` -> `evaluator.md`, `prompts/phase-team.md` -> `phase-team.md`, `references/workflow-patterns.md` -> `workflow-patterns.md`, `prompts/council-gate.md` -> `council-gate.md`, `scripts/repo-state.sh` -> `repo-state.sh`, en `references/repo-state-comparison.md` + `references/goal-format.md` -> `.supergoal/references/` (PROTOCOL.md verwijst er relatief naar, dus de runtime-bundel moet ze zelf dragen).
...
Koos Stage 6.6 `/loop`: print in stap 4 een `/loop` regel in plaats van de `/goal` regel (bijv. `/loop <interval> voer de volgende ronde uit volgens .supergoal/PROTOCOL.md`); voor een vaste tijd een scheduled task. De rest van de overdracht is gelijk.

---

## Uitvoering: de fase-loop

Draait binnen de `/goal` sessie, herhaald tot `SUPERGOAL_RUN_COMPLETE`. Volledige mechaniek in `.supergoal/PROTOCOL.md`; de kern:
